...
__all__ = ["DataLoader", "BaseCollateFunction", "BatchImageCollateFunction", "batch_image_collate_fn"]
        ...
        if self.mixup_prob > 0:
            ...
            print(
                "     ### Using MixUp with Prob@{} in {} epochs ### ".format(
                    self.mixup_prob, self.mixup_epochs
                )
            )
        ...
        if random.random() < self.mixup_prob and self.mixup_epochs[0] <= self.epoch < self.mixup_epochs[-1]:
            ...
            if self.data_vis:
                for i in range(len(updated_targets)):
                    ...
                    pilImage.save(
                        self.vis_save + str(i) + "_" + str(len(updated_targets[i]["boxes"])) + "_out.jpg"
                    )
