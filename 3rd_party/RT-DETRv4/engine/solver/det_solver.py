...
class DetSolver(BaseSolver):
    def fit(
        ...
        if self.last_epoch > 0:
            ...
            test_stats, coco_evaluator = evaluate(
                module, self.criterion, self.postprocessor, self.val_dataloader, self.evaluator, self.device
            )
            for k in test_stats:
        ...
        for epoch in range(start_epoch, args.epoches):
            ...
            if (
                ...
                and hasattr(self.criterion, "distill_adaptive_params")
                and self.criterion.distill_adaptive_params
                and self.criterion.distill_adaptive_params.get("enabled", False)
            ):
                    ...
                    if not (lower_bound <= avg_percentage <= upper_bound):
                        ...
                        if current_weight > 1e-6:
                            ...
                            if abs(denominator) >= 1e-9:
                                ...
                                new_weight = min(
                                    max(new_weight, current_weight / 10.0), current_weight * 10.0
                                )
                ...
                print(
                    f"Epoch {epoch}: avg encoder grad {avg_percentage:.2f}% | distill {current_weight:.6f} -> {new_weight:.6f} ({reason})"
                )
    ...
    def val(
        ...
        if self.output_dir:
            saved_metrics = coco_evaluator.coco_eval["bbox"].extended_metrics
            test_stats["keys"] = [
                "map@50:95_all_maxdets=100",
                "map@50_all_maxdets=100",
                "map@75_all_maxdets=100",
                "map@50:95_small_maxdets=100",
                "map@50:95_medium_maxdets=100",
                "map@50:95_large_maxdets=100",
                "AR@50:95_all_maxdets=1",
                "AR@50:95_all_maxdets=10",
                "AR@50:95_all_maxdets=100",
                "AR@50:95_small_maxdets=100",
                "AR@50:95_medium_maxdets=100",
                "AR@50:95_large_maxdets=100",
            ]
            saved_metrics["test_stats"] = test_stats
            # dist_utils.save_on_master(coco_evaluator.coco_eval["bbox"].eval, self.output_dir / "eval.pth")
            dist_utils.save_on_master(saved_metrics, self.output_dir / "eval.pth")
