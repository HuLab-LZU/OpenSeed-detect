    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if scaler is not None:
            with torch.autocast(device_type=str(device), cache_enabled=True):
                outputs = model(
                    samples, targets=targets, teacher_encoder_output=teacher_encoder_output_for_distillation
                )
            ...
            if (
                ...
                and hasattr(criterion, "distill_adaptive_params")
                and getattr(criterion, "distill_adaptive_params")
                and criterion.distill_adaptive_params.get("enabled", False)
            ):
