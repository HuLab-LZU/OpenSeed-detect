...
def yaml_model_load(path):
    ...
    sub_ptn = r"(\d+)([nslmx])(.+)?$"
    if "master" in path.stem:
        sub_ptn = r"(master)(-[nslmx])(.+)?$"
    unified_path = re.sub(sub_ptn, r"\1\3", str(path))  # i.e. yolov8x.yaml -> yolov8.yaml
...
def guess_model_scale(model_path):
    ...
    """
    try:
        if "master" in Path(model_path).stem:
            return re.search(r"master-([nslmx])", Path(model_path).stem).group(1)
